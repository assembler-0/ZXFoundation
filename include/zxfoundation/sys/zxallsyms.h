...
/// @param name_out  Buffer to receive the NUL-terminated symbol name.
///                  May be NULL if only the offset is needed.
/// @param size      Capacity of name_out in bytes.  If the name is longer
///                  than size-1 it is truncated; a NUL is always written.
/// @param offset    Receives (addr - symbol_base).  May be NULL.
...
///
/// @param name      NUL-terminated symbol name to search for.
/// @param addr_out  Receives the symbol's virtual address on success.
...
///
/// @param cb   Callback invoked for each symbol.
/// @param arg  Opaque argument forwarded to cb unchanged.
