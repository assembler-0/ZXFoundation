...
static void load_parmfile(uint32_t schid) {
    ...
    if (dasd_read_record(schid, ext.begin_cyl, ext.begin_head, 1, CCW_CMD_READ_DATA, parm_block, DASD_BLOCK_SIZE) < 0) return;
    uint32_t i = 0;
...
[[noreturn]] void zxfl01_entry(const uint32_t schid) {
    ...
    if (zxfl_load_elf64(schid, &kernel_ext, &entry_point, &load_base, &load_size, ZXVL_COMPUTE_TOKEN(s_proto.stfle_fac[0], schid)) < 0)
        panic("zxfl01: load error");
    s_proto.kernel_phys_start = load_base;
    s_proto.kernel_phys_end   = load_base + load_size;
    s_proto.kernel_entry      = entry_point;
    s_proto.mem_map_count = probe_memory(s_mem_map, ZXFL_MEM_MAP_MAX, s_proto.kernel_phys_start, s_proto.kernel_phys_end, mem_limit);
