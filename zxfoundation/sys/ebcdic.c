...
static const uint8_t ascii_to_ebcdic_table[256] = {
    ...
    0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7,  // 0x30-0x37 01234567
    0xF8, 0xF9, 0x7A, 0x5E, 0x4C, 0x7E, 0x6E, 0x6F,  // 0x38-0x3F 89:;<=>?
    0x7C, 0xC1, 0xC2, 0xC3, 0xC4, 0xC5, 0xC6, 0xC7,  // 0x40-0x47 @ABCDEFG
    0xC8, 0xC9, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6,  // 0x48-0x4F HIJKLMNO
    0xD7, 0xD8, 0xD9, 0xE2, 0xE3, 0xE4, 0xE5, 0xE6,  // 0x50-0x57 PQRSTUVW
    0xE7, 0xE8, 0xE9, 0xAD, 0xE0, 0xBD, 0x5F, 0x6D,  // 0x58-0x5F XYZ[\]^_
    0x79, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87,  // 0x60-0x67 `abcdefg
    0x88, 0x89, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96,  // 0x68-0x6F hijklmno
    0x97, 0x98, 0x99, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6,  // 0x70-0x77 pqrstuvw
    0xA7, 0xA8, 0xA9, 0xC0, 0x4F, 0xD0, 0xA1, 0x07,  // 0x78-0x7F xyz{|}~
...
/// @brief EBCDIC to ASCII conversion table (Code Page 037)
...
uint8_t ascii_to_ebcdic(uint8_t ascii) {
    return ascii_to_ebcdic_table[ascii];
}
...
void ascii_to_ebcdic_buf(char *buf, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        buf[i] = (char)ascii_to_ebcdic_table[(uint8_t)buf[i]];
    }
}

void ebcdic_to_ascii_buf(char *buf, size_t len) {
    for (size_t i = 0; i < len; ++i) {
        buf[i] = (char)ebcdic_to_ascii_table[(uint8_t)buf[i]];
    }
}
